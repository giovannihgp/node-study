...
} from './circuit-breaker.interface.js';
...
export class CircuitBreakerService {
  ...
  async executeWithCircuitBreaker<T>(
    ...
    if (circuit.state === 'OPEN') {
      if (Date.now() < circuit.nextAttemptTime) {
        this.logger.warn(`Circuit breaker OPEN for ${key}, usig fallback`);
    ...
    try {
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Circuit breaker failure for ${key}: ${error.message}`,
          error.stack,
        );
      } else {
        this.logger.error(`Circuit breaker failure for ${key}: Unknown error`);
      }
  ...
  private onFailure(
    ...
    if (circuit.failureCount >= options.failureThreshold) {
      ...
      circuit.lastFailureTime = Date.now() + options.resetTimeout;
      this.logger.warn(`
