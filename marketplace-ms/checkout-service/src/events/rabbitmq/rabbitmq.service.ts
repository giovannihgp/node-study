...
export class RabbitmqService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connect() {
    try {
      ...
      this.connection.on('close', () => {
        this.logger.warn('⚠️ RabbitMQ connection close');
      });
  ...
  async publishMessage(
    ...
    try {
      ...
      this.logger.debug(`⚠️ Message content: ${JSON.stringify(message)}`);
      if (!published) {
  ...
  async subcribeToQueue(
    ...
    try {
      ...
      this.logger.log(
        `✅ Subscribed to queue: ${queueName} with rounting key: ${routingKey}`,
      );
    } catch (error) {
